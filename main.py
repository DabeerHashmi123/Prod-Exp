...
try:
    ...
    print("win10toast is not installed. Installing win10toast...")
    try:
        import subprocess
        subprocess.check_call([sys.executable, "-m", "pip", "install", "win10toast"])
        from win10toast import ToastNotifier
    except Exception as e:
        print("Failed to install win10toast:", e)
        sys.exit(1)

try:
    import sqlite3
except ImportError:
    print("SQLite is not installed. Installing SQLite...")
    try:
        import subprocess
        subprocess.check_call([sys.executable, "-m", "pip", "install", "sqlite3"])
        import sqlite3
    except Exception as e:
        print("Failed to install SQLite:", e)
        sys.exit(1)
...
class AboutDialog(QDialog):
    def __init__(self, *args, **kwargs):
        ...
        layout.addWidget(QLabel("Copyright @elviskojo 2023"))
